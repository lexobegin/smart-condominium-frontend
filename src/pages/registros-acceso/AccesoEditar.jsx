...
function AccesoEditar() {
  ...
  const [currentFotoEvidencia, setCurrentFotoEvidencia] = useState("");
  ...
  const loadData = async () => {
    try {
      ...
      const [registroData, usuariosData, vehiculosData] = await Promise.all([
        fetchRegistroAcceso(id),
      ...
      const fechaHora = registroData.fecha_hora
        ? new Date(registroData.fecha_hora).toISOString().slice(0, 16)
        : new Date().toISOString().slice(0, 16);

      setForm({
        usuario: registroData.usuario?.id?.toString() || "",
        vehiculo: registroData.vehiculo?.id?.toString() || "",
        tipo: registroData.tipo || "peatonal",
        direccion: registroData.direccion || "entrada",
        metodo: registroData.metodo || "manual",
        fecha_hora: fechaHora,
        reconocimiento_exitoso:
          registroData.reconocimiento_exitoso !== undefined
            ? registroData.reconocimiento_exitoso
            : true,
        confidence_score: registroData.confidence_score?.toString() || "",
        foto_evidencia: null,
      });

      // Guardar URL de evidencia actual para mostrar
      setCurrentFotoEvidencia(registroData.foto_evidencia || "");

      setUsuarios(usuariosData.results || usuariosData || []);
      setVehiculos(vehiculosData.results || vehiculosData || []);
    } catch (err) {
      console.error("Error cargando datos:", err);
      setError("Error al cargar los datos del registro de acceso");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (form.vehiculo) {
        ...
        // Si no hay vehículo, asegurarse de que no se envíe el campo
        formData.append("vehiculo_id", "");
      }
      ...
      console.log(
        "Actualizando registro de acceso:",
        Object.fromEntries(formData)
      );
      await updateRegistroAcceso(id, formData);

      navigate("/registros-acceso");
    } catch (err) {
      console.error("Error actualizando registro de acceso:", err);
      const errorMessage =
        err.response?.data?.message ||
        err.response?.data?.detail ||
        "Error al actualizar el registro de acceso";
  ...
  return (
    <DashboardLayout>
      <div className="d-flex align-items-center mb-4">
        ...
        <h2 className="mb-0">Editar Registro de Acceso</h2>
      </div>
      ...
      <Card className="mb-4">
        <Card.Body className="bg-light">
          <small>
            <strong>ID del Registro:</strong> {id} |
          ...
          <Col md={6}>
            <Form.Group>
              ...
              <Form.Control
                ...
                onChange={handleChange}
              />
              {currentFotoEvidencia && !form.foto_evidencia && (
                <div className="mt-2">
                  <small className="text-muted">
                    Evidencia actual:
                    <a
                      href={currentFotoEvidencia}
                      target="_blank"
